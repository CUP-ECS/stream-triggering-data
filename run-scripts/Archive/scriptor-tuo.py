...
def main():
    ...
    parser.add_argument("--times", type=str, default="all-01:30:00")
    ...
    partition = "pdebug"
    cycles = args.cycles
    # all-1-30 -> every file has time #SBATCH --time=01:30:00
    # otherwise list times for each node
    time_arg = args.times
    times = []
    if "all" in time_arg:
        x, y = time_arg.split("-")
        times = [y for i in nodes]
    else:
        ...
        with open(outfile, 'w') as scriptfile:
            ...
            for mem_type in finegrain:
                ...
                for base in base_nums:
                    ...
                    #scriptfile.write(f'export i={node}\n')
                    scriptfile.write(ntasks)
                    scriptfile.write(f'\t\techo "Solver specs: n:{node} gpus:$j"\n')
                    #scriptfile.write("""\tfor k in {1..5}; do\n""")

                    # mpi advance
                    scriptfile.write("""\t\techo "Solver backend: MPIAdvance CXI Double Buffering"\n""")
                    scriptfile.write(f"""\t\techo "Solver base: $base"\n""")
                    scriptfile.write(f'\t\tsrun --nodes={node} --ntasks-per-node=$j {gol_file} -n {base} -c mpi-advance -t {cycles}\n')
                    ...
                    scriptfile.write(f"""\t\tsrun --nodes={node} --ntasks-per-node=$j {gol_file} -n {base} -c mpi -t {cycles}\n""")
